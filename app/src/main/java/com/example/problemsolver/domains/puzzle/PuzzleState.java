...
//        return tilesOutOfPlace(this, goal);
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        ...
        for (int i =1 ; i<9; i++){
            if(!((PuzzleState)current).getLocation(i).equals(((PuzzleState)goal).getLocation(i))) count++;
        }
    ...
    public static int sumManhattan(State current, State goal) {
        int sum = 0;
        for (int i =1 ; i<9; i++){
            if(!((PuzzleState)current).getLocation(i).equals(((PuzzleState)goal).getLocation(i))) sum+= Msum(((PuzzleState)current).getLocation(i), ((PuzzleState)goal).getLocation(i));
        }
        return sum;
    }

    private static int Msum(Location cur, Location goa){
        int x =Math.abs(cur.getRow()-goa.getRow());
        int y = Math.abs(cur.getColumn()- goa.getColumn());
        return x+y;
    }
